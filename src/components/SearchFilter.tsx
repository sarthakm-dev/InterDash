...
const SearchFilter = ({ data, onFilter, theme, counter }: SearchFilterProps) => {
  ...
  useEffect(() => {
    if (query) {
      ...
      setSearchHistory(prev => [...prev, { query, resultCount: filtered.length, time: Date.now() }])
      const saved = JSON.parse(localStorage.getItem('searchHistory') || '[]')
      saved.push({ query, resultCount: filtered.length, time: Date.now() })
    ...
    <Card>
      ...
      <CardContent>
        ...
        <div className="flex gap-2 mt-2 text-xs text-muted-foreground">
          ...
          {searchHistory.length > 0 && <Badge variant="secondary">History: {searchHistory.length} queries</Badge>}
        </div>
